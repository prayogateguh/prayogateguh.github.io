...
const navNotemapBtn = document.getElementById("nav-notemap");
...
const appNotemapContainer = document.getElementById("app-notemap");
...
function switchApp(appName) {
  ...
  appNotemapContainer.classList.toggle("hidden", appName !== "notemap");

  const updateBtnStyles = (btn, isActive) => {
    ...
    btn.classList.toggle("dark:hover:bg-gray-700/50", !isActive);
  };

  updateBtnStyles(navPlannerBtn, appName === "planner");
  updateBtnStyles(navTrackerBtn, appName === "tracker");
  updateBtnStyles(navNotemapBtn, appName === "notemap");

  if (appName === "notemap" && !mindMapInstance) {
...
navNotemapBtn.addEventListener("click", () => switchApp("notemap"));
...
document
  .getElementById("confirmation-confirm-btn")
  .addEventListener("click", () => {
    if (confirmationCallback) confirmationCallback();
    closeConfirmationModal();
  });
document
  .getElementById("confirmation-cancel-btn")
...
window.eisenhowerPlannerAPI = (function eisenhowerPlanner() {
  ...
  function getFileFromDB(id) {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = (event) =>
        reject("Error fetching file: " + event.target.error);
    });
  }
  ...
  let appPlannerContainer,
    grid,
    workspaceManager,
    unsortedColumnWrapper,
    unsortedContainer,
    currentDateEl,
    todayBtn,
    prevDayBtn,
  ...
  function addTask(quadrantId, text) {
    const activeWS = appData.workspaces.find(
      (ws) => ws.id === appData.activeWorkspaceId
    );
    if (!activeWS) return;
  ...
  function loadData() {
    ...
    const storedUiState = JSON.parse(
      localStorage.getItem("eisenhowerUiStateV2")
    );
    if (storedUiState) {
  ...
  function applyUiState() {
    ...
    const desktopToggleBtn = document.getElementById(
      "desktop-toggle-unsorted-btn"
    );
    const mobileToggleBtn = unsortedQuadrant.querySelector(
      "#toggle-unsorted-btn"
    );
    const collapsibleContent = unsortedQuadrant.querySelector(
      ".collapsible-content"
    );
  ...
  function renderDate() {
    ...
    currentDateEl.textContent = uiState.currentDate.toLocaleDateString(
      undefined,
      options
    );
  }
  ...
  function renderWorkspaces() {
    const activeWorkspace = appData.workspaces.find(
      (ws) => ws.id === appData.activeWorkspaceId
    );
    if (!activeWorkspace) {
      workspaceManager.innerHTML =
        '<div class="font-bold text-gray-900 dark:text-white">No Workspace</div>';
      return;
    }
                      ...
                      <div class="p-2">
                          ...
                          <div id="workspace-dropdown-list" class="flex flex-col gap-1">
                              ${appData.workspaces
                                .map(
                                  (ws) => `
                                  <div class="workspace-menu-item-group flex items-center justify-between group rounded-md ${
                                    ws.id === appData.activeWorkspaceId
                                      ? "bg-indigo-100 dark:bg-indigo-600/30"
                                      : "hover:bg-gray-100 dark:hover:bg-gray-700/50"
                                  }">
                                       <button data-id="${
                                         ws.id
                                       }" class="workspace-btn flex-grow text-left px-2 py-1.5 text-sm font-medium rounded-l-md ${
                                    ws.id === appData.activeWorkspaceId
                                      ...
                                      : "text-gray-800 dark:text-gray-300"
                                       ...
                                       <div class="sidebar-item-actions flex items-center pr-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button data-id="${
                                              ws.id
                                            }" class="edit-workspace-btn p-1.5 rounded hover:bg-gray-300 dark:hover:bg-gray-600/80 focus-ring "><svg class="w-4 h-4 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button>
                                            ${
                                              appData.workspaces.length > 1
                                                ? `<button data-id="${ws.id}" class="delete-workspace-btn p-1.5 rounded hover:bg-gray-300 dark:hover:bg-gray-600/80 focus-ring"><svg class="w-4 h-4 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>`
                                                : ""
                                            }
                                           </div>
                                  </div>
  ...
  function renderQuadrants() {
    ...
    quadrantData.forEach((q) => {
      ...
      quadrantEl.innerHTML = `
                        <div class="flex justify-between items-center mb-3 p-3 rounded-t-2xl ${
                          q.headerBg
                        }">
                            <div>
                                ...
                                <p class="text-sm ${q.color} opacity-80">${
        q.desc
      }</p>
                            </div>
                            <div class="flex items-center gap-2 relative">
                                ${
                                  isUnsorted
                                    ? `<button id="toggle-unsorted-btn" title="Collapse" class="p-1 rounded-full hover:bg-black/10 text-gray-500 dark:text-gray-400 md:hidden"><svg class="w-5 h-5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></button>`
                                    : ""
                                }
                            </div>
                        </div>
                        <div class="collapsible-content flex flex-col flex-grow min-h-0 px-4 pb-4">
                            ...
                            <div class="tasks-container flex-grow overflow-y-auto space-y-2 pr-2"></div>
                             <div class="add-task-container mt-auto pt-4">
                                 <button class="add-task-btn w-full text-left flex items-center gap-2 p-2 rounded-lg border-2 border-dashed ${
                                   q.border
                                 } ${q.hover} ${
        q.color
                                 ...
                                 <form class="add-task-form hidden" data-quadrant-id="${
                                   q.id
                                 }">
      ...
      quadrantEl.addEventListener("drop", (e) => {
        ...
        quadrantEl.classList.remove("drag-over");
        document
          .querySelectorAll(".task-drag-over-top, .task-drag-over-bottom")
          .forEach((el) =>
        ...
        if (
          sourceQuadrantId === targetQuadrantId &&
          draggedItem.taskIndex < dropIndex
        )
          dropIndex--;
  ...
  function renderPastTasks(quadrantId, allTasks) {
    ...
    const pastTasksContainer = quadrantEl.querySelector(
      ".past-tasks-container"
    );
    const pastTasksList = pastTasksContainer.querySelector(".past-tasks-list");
    const pastTaskItemsContainer = pastTasksContainer.querySelector(
      ".past-task-items-container"
    );
    ...
    if (pastTasks.length > 0) {
      ...
      pastTasks.forEach((task) => {
        ...
        div.className =
          "flex items-center justify-between p-2 bg-gray-100 dark:bg-gray-700/50 rounded-md";
  ...
  function renderTasks() {
    ...
    if (!activeWorkspace.tasks[currentDateStr]) {
      activeWorkspace.tasks[currentDateStr] = JSON.parse(
    ...
    for (const quadrantId in defaultTasks) {
      ...
      let taskList =
        activeWorkspace.tasks[currentDateStr] &&
        activeWorkspace.tasks[currentDateStr][quadrantId]
          ? activeWorkspace.tasks[currentDateStr][quadrantId]
          : [];
      ...
      if (taskList.length === 0) {
        ...
        taskList.forEach((task, index) => {
          ...
          tasksContainer.appendChild(
            createTaskElement(task, quadrantId, originalIndex)
          );
        });
      }
  ...
  function createTaskElement(task, quadrantId, index) {
    const div = document.createElement("div");
    div.className =
      "task-item bg-white dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600/50 p-3 rounded-lg flex items-start justify-between group shadow-sm";
    ...
    div.innerHTML = `
                        <div class="flex items-start gap-3 flex-grow min-w-0">
                            ...
                            <div class="task-details-trigger flex-grow min-w-0 cursor-pointer">
                                <div class="flex items-center gap-2">
                                    <p class="task-text text-sm flex-grow ${
                                      task.done
                                ...
                                <div class="flex items-center flex-wrap gap-x-3 gap-y-1 text-xs mt-1">
                                    ${
                                      subtaskProgress
                                        ? `<span class="flex items-center gap-1 text-gray-500 dark:text-gray-400"><svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M3 5a2 2 0 002-2h10a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V5zm2-1a1 1 0 00-1 1v2h12V5a1 1 0 00-1-1H5zM4 9v6a1 1 0 001 1h10a1 1 0 001-1V9H4z" clip-rule="evenodd" /></svg><span>${subtaskProgress}</span></span>`
                                        : ""
                                    }
                                    ${
                                      attachmentCount > 0
                                        ? `<span class="flex items-center gap-1 text-gray-500 dark:text-gray-400"><svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8 4a3 3 0 00-3 3v4a3 3 0 106 0V7a1 1 0 112 0v4a5 5 0 11-10 0V7a3 3 0 013-3z" clip-rule="evenodd" /></svg><span>${attachmentCount}</span></span>`
                                        : ""
                                    }
                                    ${deadlineHtml}
                                </div>`
    ...
    div
      .querySelector(".task-details-trigger")
      .addEventListener("click", () => openTaskDetailsModal(quadrantId, index));
  ...
  window.openWorkspaceModal = function (id = null) {
    ...
    editingTaskLocation = { ...editingTaskLocation, editingWorkspaceId: id };
    if (id) {
      workspaceModal.querySelector("#workspace-modal-title").textContent =
        "Edit Workspace";
  ...
  window.closeWorkspaceModal = function () {
    ...
    editingTaskLocation = { ...editingTaskLocation, editingWorkspaceId: null };
  };
  ...
  window.openTaskDetailsModal = function (quadrantId, index) {
    ...
    const task = appData.workspaces.find(
      (ws) => ws.id === appData.activeWorkspaceId
    ).tasks[uiState.currentDate.toISOString().split("T")[0]][quadrantId][index];

    taskDetailsModal.querySelector("#task-details-title").value = task.text;
    taskDetailsModal.querySelector("#task-details-status").value =
      task.status || "draft";
    taskDetailsModal.querySelector("#task-details-deadline").value =
      task.deadline || "";
    taskDetailsModal.querySelector("#task-details-description").value =
      task.description || "";
  ...
  window.closeTaskDetailsModal = function () {
    if (editingTaskLocation) {
      ...
      const todayTasks =
        activeWS.tasks[uiState.currentDate.toISOString().split("T")[0]];
      if (
        todayTasks &&
        todayTasks[quadrantId] &&
        todayTasks[quadrantId][index]
      ) {
        const task = todayTasks[quadrantId][index];
        task.text = taskDetailsModal
          .querySelector("#task-details-title")
          .value.trim();
        task.status = taskDetailsModal.querySelector(
          "#task-details-status"
        ).value;
        const deadlineValue = taskDetailsModal.querySelector(
          "#task-details-deadline"
        ).value;
        task.deadline = deadlineValue ? deadlineValue : null;
        task.description = taskDetailsModal
          .querySelector("#task-details-description")
          .value.trim();
        saveData();
        renderTasks();
      }
  ...
  async function openMediaPreview(attachment) {
    if (!db) {
      alert(
        "Preview is not available as the database could not be initialized."
      );
      return;
    }
    try {
  ...
  async function handleFileAttachment(e) {
    ...
    for (const file of files) {
      ...
      try {
        if (db) {
          // Only save to DB if it was initialized
  ...
  function renderAttachments(attachments) {
    ...
    attachments.forEach((attachment, index) => {
      const div = document.createElement("div");
      div.className =
        "attachment-item flex items-center justify-between p-2 bg-gray-100/60 dark:bg-gray-700/50 rounded-md group";
      div.innerHTML = `
  ...
  function renderSubTasks(subTasks) {
    ...
    subTasks.forEach((subtask, index) => {
      const div = document.createElement("div");
      div.className =
        "subtask-item flex items-center gap-3 bg-gray-100/60 dark:bg-gray-700/50 p-2 rounded-md group";
      div.dataset.index = index;
      div.draggable = true;
      div.innerHTML = `
                        ...
                        <input type="checkbox" class="subtask-checkbox h-4 w-4 rounded border-gray-400 dark:border-gray-500 bg-gray-300 dark:bg-gray-700 text-indigo-500 flex-shrink-0 focus:ring-indigo-500" ${
                          subtask.done ? "checked" : ""
                        }>
                        <span class="subtask-text flex-grow ${
                          subtask.done
  ...
  function bindPlannerEventListeners() {
    appPlannerContainer.addEventListener("click", (e) => {
      ...
      if (
        button.id === "desktop-toggle-unsorted-btn" ||
        button.id === "toggle-unsorted-btn"
      ) {
      ...
      if (button.classList.contains("delete-workspace-btn")) {
        const workspaceId = button.dataset.id;
        openConfirmationModal(
          "Are you sure you want to delete this workspace and all its tasks?",
          () => {
            appData.workspaces = appData.workspaces.filter(
    ...
    document.addEventListener("click", (e) => {
      if (
        !calendarPopover.classList.contains("hidden") &&
    ...
    const handleWorkspaceSave = () => {
      ...
      const editingWorkspaceId =
        editingTaskLocation && editingTaskLocation.editingWorkspaceId;

      if (editingWorkspaceId) {
        const existingWorkspace = appData.workspaces.find(
          (ws) => ws.id === editingWorkspaceId
        );
...
window.subscriptionTrackerAPI = (function subscriptionTracker() {
  ...
  function loadSubscriptions() {
    const storedSubscriptions = JSON.parse(
      localStorage.getItem("subscriptionTrackerV2")
    );
    ...
    const storedSettings = JSON.parse(
      localStorage.getItem("subscriptionTrackerSettingsV1")
    );
    if (storedSettings) {
    ...
    if (d < new Date()) {
      ...
      while (d < today) {
        switch (frequency_unit) {
          case "days":
            d.setDate(d.getDate() + frequency_count);
            break;
          case "weeks":
            d.setDate(d.getDate() + frequency_count * 7);
            break;
          case "months":
            d.setMonth(d.getMonth() + frequency_count);
            break;
          case "years":
            d.setFullYear(d.getFullYear() + frequency_count);
            break;
          default:
  ...
  function getPaymentsForMonth(sub, year, month) {
    ...
    if (paymentDate > calendarMonthEndDate) {
      ...
      while (paymentDate > calendarMonthStartDate) {
        ...
        switch (sub.frequency_unit) {
          case "days":
            tempDate.setDate(tempDate.getDate() - sub.frequency_count);
            break;
          case "weeks":
            tempDate.setDate(tempDate.getDate() - sub.frequency_count * 7);
            break;
          case "months":
            tempDate.setMonth(tempDate.getMonth() - sub.frequency_count);
            break;
          case "years":
            tempDate.setFullYear(tempDate.getFullYear() - sub.frequency_count);
            break;
        }
    ...
    while (paymentDate <= calendarMonthEndDate) {
      if (
        paymentDate >= calendarMonthStartDate &&
        ...
        if (isValidPayment) {
          payments.push({
            ...sub,
            payment_date_in_month: new Date(paymentDate),
          });
        }
      }
      ...
      switch (sub.frequency_unit) {
        case "days":
          paymentDate.setDate(paymentDate.getDate() + sub.frequency_count);
          break;
        case "weeks":
          paymentDate.setDate(paymentDate.getDate() + sub.frequency_count * 7);
          break;
        case "months":
          paymentDate.setMonth(paymentDate.getMonth() + sub.frequency_count);
          break;
        case "years":
          paymentDate.setFullYear(
            paymentDate.getFullYear() + sub.frequency_count
          );
          break;
  ...
  function renderCalendarView(filteredSubscriptions) {
    ...
    calendarMonthYear.textContent = calendarDate.toLocaleDateString("default", {
      month: "short",
      year: "numeric",
    });
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      dayEl.className =
        "relative border-t border-gray-200 dark:border-gray-700/50 p-1 min-h-[100px]";
      dayEl.innerHTML = `<div class="text-sm text-center ${
        isToday
          ? "font-bold text-white bg-indigo-600 rounded-full w-6 h-6 flex items-center justify-center mx-auto"
          : ""
      }">${day}</div>`;
      ...
      if (paymentsByDay[day]) {
        ...
        paymentsByDay[day].slice(0, 3).forEach((p) => {
          ...
          paymentEl.className =
            "text-xs bg-indigo-100 dark:bg-indigo-600/30 p-1 rounded flex justify-between items-center";
          paymentEl.innerHTML = `
                    ...
                    `;
          paymentEl.title = `${p.name} - ${
            p.currency || currencySymbol
          }${formatNumber(p.price)}`;
          paymentsList.appendChild(paymentEl);
        });
        if (paymentsByDay[day].length > 3) {
          paymentsList.innerHTML += `<div class="text-xs text-gray-500">+${
            paymentsByDay[day].length - 3
          } more</div>`;
        }
  ...
  function getMonthlyCost(sub) {
    ...
    switch (sub.frequency_unit) {
      case "days":
        return (sub.price / count) * 30.44;
      case "weeks":
        return (sub.price / count) * 4.33;
      case "months":
        return sub.price / count;
      case "years":
        return sub.price / (count * 12);
      default:
        return 0;
    }
  }
  ...
  function updateSummary(filteredSubscriptions) {
    ...
    if (upcoming.length > 0) {
      ...
      const daysUntil = Math.ceil(
        (new Date(next.next_payment_date) - new Date()) / (1000 * 60 * 60 * 24)
      );
      upcomingBillingHtml = `
                <span class="font-semibold">${
                  next.name
                }</span> is next in <span class="font-bold text-indigo-500">${daysUntil}</span> days (${
        next.currency || currencySymbol
      }${formatNumber(next.price)})
            `;
    }
  ...
  function createSubscriptionListItem(sub) {
    ...
    item.className =
      "subscription-list-item bg-white dark:bg-gray-800/60 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700/80 p-4 flex items-center justify-between";
    ...
    const frequency = `every ${sub.frequency_count || 1} ${
      sub.frequency_unit || "month(s)"
    }`;

    item.innerHTML = `
                    <div class="flex items-center gap-4 flex-grow min-w-0">
                        <div class="w-10 h-10 rounded-md bg-gray-100 dark:bg-gray-700 flex items-center justify-center flex-shrink-0">
                             <img src="https://logo.clearbit.com/${sub.name
                               .toLowerCase()
                               .replace(/\s+/g, "")}.com" alt="${
      sub.name
    }" class="w-6 h-6 object-contain" onerror="this.style.display='none'; this.nextElementSibling.classList.remove('hidden');">
                             <span class="hidden text-lg font-bold text-gray-500">${sub.name
                               .charAt(0)
                               .toUpperCase()}</span>
                        </div>
      ...
      sub.status === "active"
        ...
        : "Cancelled"
                        ...
                        ${
                          sub.status === "active"
                            ? isOverdue
                              ...
                              : `<p class="text-xs text-gray-500">(${daysUntil} days)</p>`
                            : ""
                        }
                    </div>
                    <div class="flex items-center justify-end gap-2 flex-shrink-0">
                        <button data-id="${
                          sub.id
                        }" class="edit-subscription-btn p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 focus-ring"><svg class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button>
                        <button data-id="${
                          sub.id
                        }" class="delete-subscription-btn p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 focus-ring"><svg class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                    </div>
                `;
    return item;
  }
  ...
  function openSubscriptionModal(id = null) {
    ...
    document
      .getElementById("cancellation-date-container")
      .classList.add("hidden");
    editingSubscriptionId = id;
    if (id) {
      subscriptionModal.querySelector("#subscription-modal-title").textContent =
        "Edit Subscription";
      ...
      document.getElementById("subscription-currency").value =
        sub.currency || "USD";
      document.getElementById("subscription-frequency-count").value =
        sub.frequency_count || 1;
      document.getElementById("subscription-frequency-unit").value =
        sub.frequency_unit || "months";
      ...
      document.getElementById("subscription-next-payment").value = new Date(
        paymentDate
      )
        .toISOString()
        .split("T")[0];
      document.getElementById("subscription-payment-method").value =
        sub.payment_method || "";
      ...
      if (sub.status === "cancelled") {
        ...
        document.getElementById("subscription-cancellation-date").value =
          sub.cancellation_date
            ? new Date(sub.cancellation_date).toISOString().split("T")[0]
            : "";
      }
    } else {
      subscriptionModal.querySelector("#subscription-modal-title").textContent =
  ...
  function applyViewMode() {
    ...
    document
      .getElementById("tracker-list-controls")
  ...
  function bindSubscriptionEventListeners() {
    ...
    subscriptionForm.addEventListener("submit", (e) => {
      ...
      const firstPaymentDate = new Date(
        document.getElementById("subscription-next-payment").value
      );
      const frequencyUnit = document.getElementById(
      ...
      const subData = {
        ...
        next_payment_date: getNextPaymentDate(
          ...
          frequencyUnit,
          frequencyCount
        ), // Calculate the true next payment date
        payment_method: document.getElementById("subscription-payment-method")
          .value,
        status: status,
        cancellation_date:
          status === "cancelled"
    ...
    document
      .getElementById("subscription-status")
      .addEventListener("change", (e) => {
        document
          .getElementById("cancellation-date-container")
          .classList.toggle("hidden", e.target.value !== "cancelled");
      });
  ...
  function init() {
    ...
    subscriptionCalendarView = document.getElementById(
